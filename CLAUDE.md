...
- **視覺**：Emotion-LLaMA（情緒與行為證據，port 7889）
...
└── UI_API/
    ...
    ├── ai_services.py              ← Whisper、Ollama、Gemini、TTS、Emotion-LLaMA 呼叫
...
- **ai_services.py**：所有 AI 呼叫（Whisper、Ollama、Gemini、TTS、Emotion-LLaMA）集中在此。
...
POS 操作事件 → interaction_event_service（計算 risk_score）
  ...
  → triggered_multimodal_analysis（Whisper + Emotion-LLaMA）
