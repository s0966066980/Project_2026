...
def mark_latest_intervention_checkout(
    ...
    open_log = (
        interaction_event_repository.find_latest_open_intervention_scoped(scope, session_id)
        if scope is not None
        else interaction_event_repository.find_latest_open_intervention(session_id)
    )
    if not open_log:
        return None
    ...
    if scope is not None:
        return interaction_event_repository.update_intervention_result_scoped(intervention_id, result, scope)
...
async def process_checkout(
    ...
    if not log_entry.get("skipped"):
        intervention_result = await asyncio.to_thread(
            mark_latest_intervention_checkout, session_id, True, cart_list, scope
        )
    else:
    ...
    try:
        finalize_args = (
            ...
            cart_list,
            cart_total,
            bool(log_entry.get("is_success", False)),
            cart_items,
        )
