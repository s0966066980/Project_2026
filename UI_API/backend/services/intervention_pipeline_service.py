...
async def run_intervention_pipeline(
    ...
    if isinstance(recent_events, list):
        ...
        events = await asyncio.to_thread(
            interaction_event_repository.get_recent_session_events_scoped,
            scope,
            safe_session_id,
        )
    else:
    ...
    barrier_scenario = scenario_service.infer_scenario_from_barrier_state(barrier_result.get("barrier_state", ""))
    ...
    if barrier_result.get("barrier_state") != "normal_operation":
        ...
        if scope is None:
            ...
            intervention_log = await asyncio.to_thread(
                interaction_event_repository.append_intervention_log_scoped,
                log_payload,
                scope,
            )
