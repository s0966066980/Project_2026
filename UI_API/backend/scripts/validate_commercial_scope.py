...
def collect_violations() -> list[Violation]:
    ...
    checks = (
        ...
        ("orders", "missing_tenant_scope", "tenant_id IS NULL"),
        ("orders", "missing_store_scope", "store_id IS NULL"),
        ("orders", "missing_device_scope", "origin_device_id IS NULL"),
        (
            "orders",
            "orphan_or_mismatched_scope",
            "NOT EXISTS (SELECT 1 FROM devices WHERE devices.id = orders.origin_device_id AND devices.store_id = orders.store_id AND devices.tenant_id = orders.tenant_id)",
        ),
        (
            "order_outbox",
            "orphan_or_mismatched_scope",
            "NOT EXISTS (SELECT 1 FROM orders WHERE orders.id = order_outbox.aggregate_id AND orders.store_id = order_outbox.store_id AND orders.tenant_id = order_outbox.tenant_id)",
        ),
