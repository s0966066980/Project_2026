...
- `.env`
- `learning_data/*.json`
- `learning_data/customer_service_media/`
- `chroma_db/`
- `chroma_db_versions/`
- `models/yolo/*.pt`
- Emotion-LLaMA 權重檔
...
本專案根目錄 `.gitignore` 已設定主要忽略規則。提交前建議確認：
...
- `/api/triggered_multimodal_analysis` 後端已完成，但前端尚未完整自動串接短片段上傳流程。
- 介入動作目前以 JSON 與簡易提示為主，尚未完成完整 UI 自動改版。
- Emotion-LLaMA 與 Whisper 推論效能仍受本機 GPU、音訊品質與片段長度影響。
- Gemini API 受 quota、模型服務狀態與網路延遲影響；推播與 RAG 審查仍固定使用本地 Ollama，以降低外部 API 依賴。
