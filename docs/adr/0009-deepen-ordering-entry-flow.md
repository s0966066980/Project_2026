...
An Ordering Session is created and bound idempotently only after the customer chooses guest entry or completes member identification or registration. Member Found, Not Found, and Service Unavailable are distinct results. Not Found offers registration or phone correction and never opens registration automatically. Member login and registration screens offer retry and a return to mode selection rather than a guest shortcut; guest entry remains the one explicit choice on the initial ordering-mode screen. Registration requires explicit acceptance of necessary terms, while order-history and personalization consent are independent, optional, and unchecked by default.
