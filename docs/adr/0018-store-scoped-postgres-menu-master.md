...
Identity is system-assigned and immutable after create (seeded ids from the JSON import are preserved). “Delete” is [[Menu Item Retirement]]—hidden from sellable surfaces and recoverable by Admin—not a hard purge, so order history and push-copy references stay addressable. Uploaded images are normalized server-side into object storage; external http(s) URLs remain valid for import compatibility. Access is now governed by [[Device-Authenticated Admin Access]] as superseded by ADR-0058.
