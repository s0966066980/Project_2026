...
> Snapshot: 2026-08-12. This is an evidence ledger, not a completion claim.
...
| 2 | Fresh, upgrade, reapply migration and checksum reconciliation | Exact candidate image applied a fresh temporary PostgreSQL database twice: both runs `28 applied / 0 pending`, all migration states `applied`, no checksum mismatch; a final-candidate custom-format dump was restored into a second temporary database and `status --require-clean` returned `28 applied / 0 pending`; temporary databases were removed | PARTIAL | Pilot-owned existing-upgrade/backup reconciliation and production restore authority remain outside this runner. |
| 3 | Python 3.10/3.12, Ruff, format, mypy, all tests | Candidate Python 3.10 suite: 285 app tests + 71 isolated sidecar/proposal tests = 356 passed; the prior Python 3.12 read-only suite passed 355 tests, and the new pure Dockerfile-context regression passed in the candidate test image; candidate Ruff check and format check pass; capability/Optimization mypy passed for 44 files | PARTIAL | Release CI attestation and a fresh Python 3.12 rerun for the new regression remain outside this local runner. |
...
| 19 | Legacy telemetry is zero | Production route import and frontend transport static gates; obsolete identity shims removed; monolithic `v1_routes.py` split into capability-owned modules; Admin settings, diagnostics, session/log controls, push-copy, Project Analyst, and Kiosk flows now use versioned capability clients; remaining compatibility consumers are enumerated in the P7 inventory | PARTIAL | Runtime telemetry must be observed as zero over complete Admin/Kiosk flows; compatibility endpoint consumers and adapters still require migration evidence. |
| 20 | Ten Module Independence ledgers and Admin/Kiosk 2/2 | Ten published capability packages, versioned route ownership tests, Identity/Operations/Recommendation transport boundary tests, 93-path candidate catalog, and frontend checks | PARTIAL | Full data-authority, PostgreSQL, restart, E2E, consumer-zero, and per-module ledgers are not yet passed. |
...
Capability/Optimization mypy: 44 files passed
...
Exact PostgreSQL-backed runtime restart: `/live` + `/ready` passed before and after restart; 20/20 adapters, migration head 0028
Frontend syntax/typecheck/build: passed
Frontend Vitest: 127 passed; coverage 93.18% statements, 80.57% branches, 94.91% functions, 93.30% lines
...
Candidate OpenAPI surface: 93 /api/v1 paths
